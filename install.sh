...
EOF
  ...
  if [ -n "$K3S_TOKEN" ]; then
    if [ -n "$K3S_URL" ]; then
      ...
      curl -sfL https://get.k3s.io | INSTALL_K3S_VERSION="$K3S_VERSION" sh -s - server --server "$K3S_URL" --tls-san="$k3s_tls_san"
      ...
      curl -sfL https://get.k3s.io | INSTALL_K3S_VERSION="$K3S_VERSION" sh -s - server --cluster-init --tls-san="$k3s_tls_san"
    fi
  else
